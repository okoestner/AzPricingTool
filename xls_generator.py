...
import datetime
import xlsxwriter # https://pypi.org/project/XlsxWriter/, https://xlsxwriter.readthedocs.io/
import sys
# import os
...
installation_dir = '/mnt/c/Users/olafk/source/repos/AzPricingTool/'
workbookNamePattern = '/mnt/c/Users/olafk/source/repos/AzPricingTool/TSI-AzQuote-{}.xlsx'
regions = ['germany-north', 'germany-west-central', 'south-africa-north', 'south-africa-west', 'switzerland-north',
           'switzerland-west', 'uae-central', 'uae-north', 'asia-pacific-east', 'asia-pacific-southeast',
           'australia-central', 'australia-central-2', 'australia-east', 'australia-southeast', 'brazil-south',
           'canada-central', 'canada-east', 'central-india', 'europe-north', 'europe-west', 'france-central',
           'france-south', 'germany-central', 'germany-northeast', 'japan-east', 'japan-west', 'korea-central',
           'korea-south', 'south-india', 'united-kingdom-south', 'united-kingdom-west', 'us-central', 'us-east',
           'us-east-2', 'usgov-arizona', 'usgov-iowa', 'usgov-texas', 'usgov-virginia', 'us-north-central',
           'us-south-central', 'us-west', 'us-west-2', 'us-west-central', 'west-india', 'norway-east', 'norway-west']
...
# you can call this file with "xls_generator OUTPUT_FILE DIRECTORY"
if len(sys.argv) > 1:
...
# KEY COLUMN INDEX
VMNameColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['VM NAME']['index']))
CPUColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['CPUs']['index']))
memColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['Mem(GB)']['index']))
dataDiskSizeColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['DATA STORAGE']['index']))
dataDiskTypeColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['DATA STORAGE TYPE']['index']))
osDiskSizeColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['OS DISK']['index']))
regionColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['REGION']['index']))
licenseColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['LICENSE']['index']))
SAPColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['SAP']['index']))
GPUColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['GPU']['index']))
ASRColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['ASR']['index']))
hoursMonthColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['HOURS/MONTH']['index']))
bSeriesColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['USE B SERIES']['index']))
reseInsColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['RESERVED INST']['index']))
dataOKColumn = xls.getColumnLetterFromIndex(
    xls.getCustomerDataColumnPositionInExcel(xls.customerInputColumns['columns']['ALL DATA OK']['index']))
...
computePriceMatrix = priceReaderCompute.getPriceMatrix(regions, currency)
...
if numVmSizes < numVmSizesCheck:
    ...
    print("Error 1")
    print("numVmSizes=" + str(numVmSizes) + ", numVmSizesCheck=" + str(numVmSizesCheck))
    sys.exit(1)
...
print("Workbook file: " + workbookFile)

# 3 - ADD TABS
tsiTab = workbook.add_worksheet('T-Systems')
customerVMDataExcelTab = workbook.add_worksheet('Customer-VM-List')
customerVMDataExcelTab.set_tab_color('#E20074') # magenta color for this sheet since we apply data here
...
#####################   T-SYSTEMS   ####################
########################################################
tsiTab.insert_image('A1', 'media/slide.jpg')
...
lastColumnLetter = xls.getColumnLetterFromIndex(
    xls.assumptions['firstCellColumn'] + xls.assumptions['header']['width'] - 1)
...
customerVMDataExcelTab.data_validation(modeCell, {'validate': 'list',
                                                  'source': xls.assumptions['rows'][category]['validationList']})
...
for column in xls.customerInputColumns['columns']:
    ...
    try:
        ...
        customerVMDataExcelTab.data_validation(xls.customerInputColumns['firstCellRow'] + 1, columnPositon,
                                               xls.rowsForVMInput, columnPositon,
                                               {'validate': 'list', 'source': columnValidationList})
    except:
        pass
...
for rowIndex in range(1, xls.rowsForVMInput):
    formulaCheckAllInputs = formulaCheckAllInputsPattern.format(rowIndex + 1, VMNameColumn, CPUColumn, memColumn,
                                                                dataDiskSizeColumn, dataDiskTypeColumn,
                                                                osDiskSizeColumn, SAPColumn, GPUColumn, ASRColumn,
                                                                hoursMonthColumn, bSeriesColumn)
    customerVMDataExcelTab.write_formula(rowIndex, xls.getCustomerDataColumnPositionInExcel(
        xls.customerInputColumns['columns']['ALL DATA OK']['index']), formulaCheckAllInputs, selectBodyStyle)

# SET FORMAT FOR ALL DATA OK
cellRange = '{0}{1}:{0}{2}'.format(dataOKColumn, xls.customerInputColumns['firstCellRow'] + 1,
                                   str(xls.rowsForVMInput + 1))
customerVMDataExcelTab.conditional_format(cellRange, {'type': 'cell', 'criteria': 'equal to', 'value': '\"YES\"',
                                                      'format': dataOKFormat})
...
for rowIndex in range(1, xls.rowsForVMInput):
    formulaVMBaseMinPrice = formulaVMBaseMinPricePattern.format(dataOKColumn, rowIndex + 1, numVmSizes, CPUColumn,
                                                                perfGainValueCell, memColumn, regionColumn,
                                                                licenseColumn, SAPColumn, GPUColumn, bSeriesColumn,
                                                                ssdRequiredCheckColumn)
    customerVMDataExcelTab.write_formula(rowIndex, firstCalculationColumnIndex + 1, formulaVMBaseMinPrice,
                                         selectBodyStyle)

    formulaVM1YMinPrice = formulaVM1YMinPricePattern.format(dataOKColumn, rowIndex + 1, numVmSizes, CPUColumn,
                                                            perfGainValueCell, memColumn, regionColumn, licenseColumn,
                                                            SAPColumn, GPUColumn, bSeriesColumn, ssdRequiredCheckColumn)
    customerVMDataExcelTab.write_formula(rowIndex, firstCalculationColumnIndex + 3, formulaVM1YMinPrice,
                                         selectBodyStyle)

    formulaVM3YMinPrice = formulaVM3YMinPricePattern.format(dataOKColumn, rowIndex + 1, numVmSizes, CPUColumn,
                                                            perfGainValueCell, memColumn, regionColumn, licenseColumn,
                                                            SAPColumn, GPUColumn, bSeriesColumn, ssdRequiredCheckColumn)
    customerVMDataExcelTab.write_formula(rowIndex, firstCalculationColumnIndex + 5, formulaVM3YMinPrice,
                                         selectBodyStyle)

    formulaVMBaseName = formulaVMBaseNamePattern.format(dataOKColumn, rowIndex + 1,
                                                        xls.alphabet[firstCalculationColumnIndex + 1], SAPColumn,
                                                        GPUColumn, numVmSizes + 1, regionColumn)
    customerVMDataExcelTab.write_formula(rowIndex, firstCalculationColumnIndex + 0, formulaVMBaseName, selectBodyStyle)

    formulaVM1YName = formulaVM1YNamePattern.format(dataOKColumn, rowIndex + 1,
                                                    xls.alphabet[firstCalculationColumnIndex + 3], SAPColumn, GPUColumn,
                                                    numVmSizes + 1, regionColumn)
    customerVMDataExcelTab.write_formula(rowIndex, firstCalculationColumnIndex + 2, formulaVM1YName, selectBodyStyle)

    formulaVM3YName = formulaVM3YNamePattern.format(dataOKColumn, rowIndex + 1,
                                                    xls.alphabet[firstCalculationColumnIndex + 5], SAPColumn, GPUColumn,
                                                    numVmSizes + 1, regionColumn)
    customerVMDataExcelTab.write_formula(rowIndex, firstCalculationColumnIndex + 4, formulaVM3YName, selectBodyStyle)

    formulaVMYearPAYG = formulaVMYearPAYGPattern.format(dataOKColumn, rowIndex + 1, hoursMonthColumn,
                                                        xls.getVMCalculationColumn('PRICE(H) PAYG'),
                                                        xls.getCustomerDataColumn('UNITS'))
    customerVMDataExcelTab.write_formula(rowIndex, firstCalculationColumnIndex + 6, formulaVMYearPAYG, selectBodyStyle)

    formulaVMYear1YRI = formulaVMYear1YRIPattern.format(dataOKColumn, rowIndex + 1,
                                                        xls.getVMCalculationColumn('PRICE(H) 1Y'),
                                                        xls.getCustomerDataColumn('UNITS'))
    customerVMDataExcelTab.write_formula(rowIndex, firstCalculationColumnIndex + 7, formulaVMYear1YRI, selectBodyStyle)

    formulaVMYear3YRI = formulaVMYear3YRIPattern.format(dataOKColumn, rowIndex + 1,
                                                        xls.getVMCalculationColumn('PRICE(H) 3Y'),
                                                        xls.getCustomerDataColumn('UNITS'))
    customerVMDataExcelTab.write_formula(rowIndex, firstCalculationColumnIndex + 8, formulaVMYear3YRI, selectBodyStyle)

    formulaBestPrice = formulaBestPricePattern.format(dataOKColumn, rowIndex + 1, reseInsColumn,
                                                      xls.getVMCalculationColumn('PAYG'),
                                                      xls.getVMCalculationColumn('3Y RI'),
                                                      xls.getVMCalculationColumn('1Y RI'))
...
for diskIndex in range(xls.managedDataDiskColumns['firstCellRow'], len(priceReaderManagedDisk.standardDiskSizes)):
    ...
    # SET HEADER
    customerVMDataExcelTab.write(xls.managedDataDiskColumns['firstCellRow'], columnIndex, dataDiskPrefix + diskName,
                                 selectHeaderStyle)
    # SET FORMULAS
    diskDataIndexInDiskTab = diskIndex + 2
    for rowIndex in range(1, xls.rowsForVMInput):
        if diskIndex == 0:
            formula = "=IF(AND({0}{1}=\"STANDARD\",{2}{1}=\"YES\",{3}{1}<='azure-standard-disk-prices'!G{4}, {3}{1}>0), ( 1+IF({5}{1}=\"YES\",1))*{6}{1},\"\")".format(
                dataDiskTypeColumn, rowIndex + 1, dataOKColumn, dataDiskSizeColumn, diskDataIndexInDiskTab, ASRColumn,
                xls.getCustomerDataColumn('UNITS'))
        else:
            formula = "=IF(AND({0}{1}=\"STANDARD\",{2}{1}=\"YES\",{3}{1}>'azure-standard-disk-prices'!G{4},{3}{1} <='azure-standard-disk-prices'!G{6}),( 1+IF({5}{1}=\"YES\",1))*{7}{1},\"\")".format(
                dataDiskTypeColumn, rowIndex + 1, dataOKColumn, dataDiskSizeColumn, diskDataIndexInDiskTab - 1,
                ASRColumn, diskDataIndexInDiskTab, xls.getCustomerDataColumn('UNITS'))
...
for diskIndex in range(xls.managedDataDiskColumns['firstCellRow'], len(priceReaderManagedDisk.premiumDiskSizes)):
    ...
    for rowIndex in range(1, xls.rowsForVMInput):
        if diskIndex == 0:
            formula = "=IF(AND({0}{1}=\"PREMIUM\",{2}{1}=\"YES\",{3}{1}<'azure-premium-disk-prices'!G{4},{3}{1}>0),   ( 1+IF({5}{1}=\"YES\",1))*{6}{1},\"\")".format(
                dataDiskTypeColumn, rowIndex + 1, dataOKColumn, dataDiskSizeColumn, diskDataIndexInDiskTab, ASRColumn,
                xls.getCustomerDataColumn('UNITS'))
        else:
            formula = "=IF(AND({0}{1}=\"PREMIUM\",{2}{1}=\"YES\",{3}{1}>'azure-premium-disk-prices'!G{4},{3}{1}<'azure-premium-disk-prices'!G{6}),( 1+IF({5}{1}=\"YES\",1))*{7}{1},\"\")".format(
                dataDiskTypeColumn, rowIndex + 1, dataOKColumn, dataDiskSizeColumn, diskDataIndexInDiskTab - 1,
                ASRColumn, diskDataIndexInDiskTab, xls.getCustomerDataColumn('UNITS'))
        customerVMDataExcelTab.write_formula(rowIndex, columnIndex, formula, selectBodyStyle)
...
# SET WIDTH
customerVMDataExcelTab.set_column(xls.ASRColumns['firstColumnIndex'], xls.ASRColumns['firstColumnIndex'],
                                  xls.ASRColumns['width'])
# SET HEADER
customerVMDataExcelTab.write(xls.ASRColumns['firstCellRow'], xls.ASRColumns['firstColumnIndex'], xls.ASRColumns['name'],
                             selectHeaderStyle)
# ROWS
...
# SET WIDTH
customerVMDataExcelTab.set_column(xls.managedS4OSDiskColumn['firstColumnIndex'],
                                  xls.managedS4OSDiskColumn['firstColumnIndex'], xls.managedS4OSDiskColumn['width'])
customerVMDataExcelTab.set_column(xls.managedP4OSDiskColumn['firstColumnIndex'],
                                  xls.managedP4OSDiskColumn['firstColumnIndex'], xls.managedP4OSDiskColumn['width'])
customerVMDataExcelTab.set_column(xls.managedS6OSDiskColumn['firstColumnIndex'],
                                  xls.managedS6OSDiskColumn['firstColumnIndex'], xls.managedS6OSDiskColumn['width'])
customerVMDataExcelTab.set_column(xls.managedP6OSDiskColumn['firstColumnIndex'],
                                  xls.managedP6OSDiskColumn['firstColumnIndex'], xls.managedP6OSDiskColumn['width'])
customerVMDataExcelTab.set_column(xls.managedS10OSDiskColumn['firstColumnIndex'],
                                  xls.managedS10OSDiskColumn['firstColumnIndex'], xls.managedS10OSDiskColumn['width'])
customerVMDataExcelTab.set_column(xls.managedP10OSDiskColumn['firstColumnIndex'],
                                  xls.managedP10OSDiskColumn['firstColumnIndex'], xls.managedP10OSDiskColumn['width'])
# SET HEADER
customerVMDataExcelTab.write(xls.managedS4OSDiskColumn['firstCellRow'], xls.managedS4OSDiskColumn['firstColumnIndex'],
                             xls.managedS4OSDiskColumn['name'], selectHeaderStyle)
customerVMDataExcelTab.write(xls.managedP4OSDiskColumn['firstCellRow'], xls.managedP4OSDiskColumn['firstColumnIndex'],
                             xls.managedP4OSDiskColumn['name'], selectHeaderStyle)
customerVMDataExcelTab.write(xls.managedS6OSDiskColumn['firstCellRow'], xls.managedS6OSDiskColumn['firstColumnIndex'],
                             xls.managedS6OSDiskColumn['name'], selectHeaderStyle)
customerVMDataExcelTab.write(xls.managedP6OSDiskColumn['firstCellRow'], xls.managedP6OSDiskColumn['firstColumnIndex'],
                             xls.managedP6OSDiskColumn['name'], selectHeaderStyle)
customerVMDataExcelTab.write(xls.managedS10OSDiskColumn['firstCellRow'], xls.managedS10OSDiskColumn['firstColumnIndex'],
                             xls.managedS10OSDiskColumn['name'], selectHeaderStyle)
customerVMDataExcelTab.write(xls.managedP10OSDiskColumn['firstCellRow'], xls.managedP10OSDiskColumn['firstColumnIndex'],
                             xls.managedP10OSDiskColumn['name'], selectHeaderStyle)
...
for rowIndex in range(1, xls.rowsForVMInput):
    ...
    formulaDiskOSStandard = formulaDiskOSS4Pattern.format(osDiskSizeColumn, rowIndex + 1, dataOKColumn, ASRColumn,
                                                          xls.getCustomerDataColumn('UNITS'))
    customerVMDataExcelTab.write_formula(rowIndex, xls.managedS4OSDiskColumn['firstColumnIndex'], formulaDiskOSStandard,
                                         selectBodyStyle)

    formulaDiskOSPremium = formulaDiskOSP4Pattern.format(osDiskSizeColumn, rowIndex + 1, dataOKColumn, ASRColumn,
                                                         xls.getCustomerDataColumn('UNITS'))
    customerVMDataExcelTab.write_formula(rowIndex, xls.managedP4OSDiskColumn['firstColumnIndex'], formulaDiskOSPremium,
                                         selectBodyStyle)

    formulaDiskOSStandard = formulaDiskOSS6Pattern.format(osDiskSizeColumn, rowIndex + 1, dataOKColumn, ASRColumn,
                                                          xls.getCustomerDataColumn('UNITS'))
    customerVMDataExcelTab.write_formula(rowIndex, xls.managedS6OSDiskColumn['firstColumnIndex'], formulaDiskOSStandard,
                                         selectBodyStyle)

    formulaDiskOSPremium = formulaDiskOSP6Pattern.format(osDiskSizeColumn, rowIndex + 1, dataOKColumn, ASRColumn,
                                                         xls.getCustomerDataColumn('UNITS'))
    customerVMDataExcelTab.write_formula(rowIndex, xls.managedP6OSDiskColumn['firstColumnIndex'], formulaDiskOSPremium,
                                         selectBodyStyle)

    formulaDiskOSStandard = formulaDiskOSS10Pattern.format(osDiskSizeColumn, rowIndex + 1, dataOKColumn, ASRColumn,
                                                           xls.getCustomerDataColumn('UNITS'))
    customerVMDataExcelTab.write_formula(rowIndex, xls.managedS10OSDiskColumn['firstColumnIndex'],
                                         formulaDiskOSStandard, selectBodyStyle)

    formulaDiskOSPremium = formulaDiskOSP10Pattern.format(osDiskSizeColumn, rowIndex + 1, dataOKColumn, ASRColumn,
                                                          xls.getCustomerDataColumn('UNITS'))
    customerVMDataExcelTab.write_formula(rowIndex, xls.managedP10OSDiskColumn['firstColumnIndex'], formulaDiskOSPremium,
                                         selectBodyStyle)

# SSD CHECK
customerVMDataExcelTab.set_column(xls.ssdRequiredCheckColumn['firstColumnIndex'],
                                  xls.ssdRequiredCheckColumn['firstColumnIndex'], xls.ssdRequiredCheckColumn['width'])
customerVMDataExcelTab.write(xls.ssdRequiredCheckColumn['firstCellRow'], xls.ssdRequiredCheckColumn['firstColumnIndex'],
                             xls.ssdRequiredCheckColumn['name'], selectHeaderStyle)
...
for rowIndex in range(1, xls.rowsForVMInput):
    ...
    customerVMDataExcelTab.write(rowIndex, xls.ssdRequiredCheckColumn['firstColumnIndex'], formulaSSDCheck,
                                 selectBodyStyle)

# ASR COST
customerVMDataExcelTab.set_column(xls.asrPriceColumn['firstColumnIndex'], xls.asrPriceColumn['firstColumnIndex'],
                                  xls.asrPriceColumn['width'])
customerVMDataExcelTab.write(xls.asrPriceColumn['firstCellRow'], xls.asrPriceColumn['firstColumnIndex'],
                             xls.asrPriceColumn['name'], selectHeaderStyle)
formulaASRPricePattern = "=IF({0}{1}=1, IFERROR(VLOOKUP({2}{1}, 'azure-asr-prices'!A$2:B${3} ,2,0)*{4}{1}, 0 ) ,\"\" )"

# DATA DISK COST
customerVMDataExcelTab.set_column(xls.diskPriceColumn['firstColumnIndex'], xls.diskPriceColumn['firstColumnIndex'],
                                  xls.diskPriceColumn['width'])
customerVMDataExcelTab.write(xls.diskPriceColumn['firstCellRow'], xls.diskPriceColumn['firstColumnIndex'],
                             xls.diskPriceColumn['name'], selectHeaderStyle)
...
# OS DISK COST
customerVMDataExcelTab.set_column(xls.osDiskPriceColumn['firstColumnIndex'], xls.osDiskPriceColumn['firstColumnIndex'],
                                  xls.osDiskPriceColumn['width'])
customerVMDataExcelTab.write(xls.osDiskPriceColumn['firstCellRow'], xls.osDiskPriceColumn['firstColumnIndex'],
                             xls.osDiskPriceColumn['name'], selectHeaderStyle)
...
# DISK AND ASR
for rowIndex in range(1, xls.rowsForVMInput):
    formulaASRPrice = formulaASRPricePattern.format(xls.getColumnLetterFromIndex(xls.ASRColumns['firstColumnIndex']),
                                                    rowIndex + 1, regionColumn, len(siteRecoveryPriceMatrix) + 1,
                                                    xls.getCustomerDataColumn('UNITS'))
    ...
    customerVMDataExcelTab.write(rowIndex, xls.diskPriceColumn['firstColumnIndex'], formulaDataDiskPrice,
                                 selectBodyStyle)

    formulaOSDiskPrice = formulaOSDiskPricePattern.format(rowIndex + 1, 600, 600)
    customerVMDataExcelTab.write(rowIndex, xls.osDiskPriceColumn['firstColumnIndex'], formulaOSDiskPrice,
                                 selectBodyStyle)
...
lastColumnLetter = xls.getColumnLetterFromIndex(
    xls.dataDiskSummary['firstCellColumn'] + xls.dataDiskSummary['header']['width'] - 1)
headerRange = '{0}{1}:{2}{1}'.format(firstColumnLetter, firstRowLetter, lastColumnLetter)
...
for column in xls.dataDiskSummary['columns']:
    customerVMDataExcelTab.write(xls.dataDiskSummary['firstCellRow'] - 1,
                                 xls.dataDiskSummary['columns'][column]['order'] - 1,
                                 xls.dataDiskSummary['columns'][column]['name'], selectHeaderStyle)
# STANDARD DATA DISKS
for index in range(0, len(priceReaderManagedDisk.standardDiskSizes)):
    customerVMDataExcelTab.write(xls.dataDiskSummary['firstCellRow'] + index, 0,
                                 priceReaderManagedDisk.standardDiskSizes[index], selectHeaderStyle)
# PREMIUM DATA DISKS
for index in range(0, len(priceReaderManagedDisk.premiumDiskSizes)):
    customerVMDataExcelTab.write(
        xls.dataDiskSummary['firstCellRow'] + len(priceReaderManagedDisk.standardDiskSizes) + index, 0,
        priceReaderManagedDisk.premiumDiskSizes[index], selectHeaderStyle)
# STANDARD DATA DISKS SUMMARY CALCULATIONS
for columnIndex in range(xls.managedDataDiskColumns['firstColumnIndex'],
                         xls.managedDataDiskColumns['firstColumnIndex'] + len(
                                 priceReaderManagedDisk.standardDiskSizes)):
    formulaCountDisk = "=SUM({0}{1}:{0}{2})".format(xls.alphabet[columnIndex],
                                                    xls.managedDataDiskColumns['firstCellRow'] + 1,
                                                    xls.rowsForVMInput + 1)
    currentCountRow = columnIndex - xls.managedDataDiskColumns['firstColumnIndex'] + xls.dataDiskSummary[
        'firstCellRow'] + 1
...
for columnIndex in range(xls.managedDataDiskColumns['firstColumnIndex'] + len(priceReaderManagedDisk.standardDiskSizes),
                         xls.managedDataDiskColumns['firstColumnIndex'] + len(
                                 priceReaderManagedDisk.standardDiskSizes) + len(
                                 priceReaderManagedDisk.premiumDiskSizes)):
    formulaCountDisk = "=SUM({0}{1}:{0}{2})".format(xls.alphabet[columnIndex],
                                                    xls.managedDataDiskColumns['firstCellRow'] + 1,
                                                    xls.rowsForVMInput + 1)
    currentCountRow = columnIndex - xls.managedDataDiskColumns['firstColumnIndex'] + xls.dataDiskSummary[
        'firstCellRow'] + 1
    currentDiskPriceRow = columnIndex - xls.managedDataDiskColumns['firstColumnIndex'] - len(
        priceReaderManagedDisk.standardDiskSizes) + 2
...
lastColumnLetter = xls.getColumnLetterFromIndex(
    xls.OSDiskSummary['firstCellColumn'] + xls.OSDiskSummary['header']['width'] - 1)
headerRange = '{0}{1}:{2}{1}'.format(firstColumnLetter, firstRowLetter, lastColumnLetter)
...
for column in xls.OSDiskSummary['columns']:
    customerVMDataExcelTab.write(xls.OSDiskSummary['firstCellRow'] - 1,
                                 xls.OSDiskSummary['columns'][column]['order'] - 1,
                                 xls.OSDiskSummary['columns'][column]['name'], selectHeaderStyle)
# PUT ROWS
S4OSDiskCountFormula = "=SUM({0}{1}:{0}{2})".format(xls.alphabet[xls.managedS4OSDiskColumn['firstColumnIndex']],
                                                    xls.managedS4OSDiskColumn['firstCellRow'] + 2,
                                                    xls.rowsForVMInput + 1)
P4OSDiskCountFormula = "=SUM({0}{1}:{0}{2})".format(xls.alphabet[xls.managedP4OSDiskColumn['firstColumnIndex']],
                                                    xls.managedP4OSDiskColumn['firstCellRow'] + 2,
                                                    xls.rowsForVMInput + 1)

S6OSDiskCountFormula = "=SUM({0}{1}:{0}{2})".format(xls.alphabet[xls.managedS6OSDiskColumn['firstColumnIndex']],
                                                    xls.managedS6OSDiskColumn['firstCellRow'] + 2,
                                                    xls.rowsForVMInput + 1)
P6OSDiskCountFormula = "=SUM({0}{1}:{0}{2})".format(xls.alphabet[xls.managedP6OSDiskColumn['firstColumnIndex']],
                                                    xls.managedP6OSDiskColumn['firstCellRow'] + 2,
                                                    xls.rowsForVMInput + 1)

S10OSDiskCountFormula = "=SUM({0}{1}:{0}{2})".format(xls.alphabet[xls.managedS10OSDiskColumn['firstColumnIndex']],
                                                     xls.managedS10OSDiskColumn['firstCellRow'] + 2,
                                                     xls.rowsForVMInput + 1)
P10OSDiskCountFormula = "=SUM({0}{1}:{0}{2})".format(xls.alphabet[xls.managedP10OSDiskColumn['firstColumnIndex']],
                                                     xls.managedP10OSDiskColumn['firstCellRow'] + 2,
                                                     xls.rowsForVMInput + 1)
...
for row in xls.OSDiskSummary['rows']:
    ...
    customerVMDataExcelTab.write(currentRowIndex, xls.OSDiskSummary['firstCellColumn'],
                                 xls.OSDiskSummary['rows'][row]['name'], selectHeaderStyle)
    customerVMDataExcelTab.write_formula(currentRowIndex, xls.OSDiskSummary['firstCellColumn'] + 1, countFormula,
                                         selectBodyStyle)
...
formulaTotalASRCost = "=12*SUM({0}{1}:{0}{2})".format(
    xls.getColumnLetterFromIndex(xls.asrPriceColumn['firstColumnIndex']), xls.ASRColumns['firstCellRow'] + 1,
    xls.rowsForVMInput + 1)
formulaTotalCost = "=SUM(B{0}:B{1})".format(xls.costSummary['firstCellRow'] + 2,
                                            xls.costSummary['firstCellRow'] + 2 + len(xls.costSummary['rows']) - 2)
# CALCULATE AND PUT HEADER
...
lastColumnLetter = xls.getColumnLetterFromIndex(
    xls.costSummary['firstCellColumn'] + xls.costSummary['header']['width'] - 1)
headerRange = '{0}{1}:{2}{1}'.format(firstColumnLetter, firstRowLetter, lastColumnLetter)
customerVMDataExcelTab.merge_range(headerRange, xls.costSummary['header']['title'], selectHeaderStyle)
# CREATE COST TOTALS
customerVMDataExcelTab.write(xls.costSummary['firstCellRow'] + xls.costSummary['rows']['COMPUTE']['order'],
                             xls.costSummary['firstCellColumn'], xls.costSummary['rows']['COMPUTE']['name'],
                             selectHeaderStyle)
customerVMDataExcelTab.write(xls.costSummary['firstCellRow'] + xls.costSummary['rows']['STORAGE']['order'],
                             xls.costSummary['firstCellColumn'], xls.costSummary['rows']['STORAGE']['name'],
                             selectHeaderStyle)
customerVMDataExcelTab.write(xls.costSummary['firstCellRow'] + xls.costSummary['rows']['ASR']['order'],
                             xls.costSummary['firstCellColumn'], xls.costSummary['rows']['ASR']['name'],
                             selectHeaderStyle)
customerVMDataExcelTab.write(xls.costSummary['firstCellRow'] + xls.costSummary['rows']['TOTAL']['order'],
                             xls.costSummary['firstCellColumn'], xls.costSummary['rows']['TOTAL']['name'],
                             selectHeaderStyle)
customerVMDataExcelTab.write_formula(xls.costSummary['firstCellRow'] + xls.costSummary['rows']['COMPUTE']['order'],
                                     xls.costSummary['firstCellColumn'] + 1, formulaTotalComputeCost, selectBodyStyle)
customerVMDataExcelTab.write_formula(xls.costSummary['firstCellRow'] + xls.costSummary['rows']['STORAGE']['order'],
                                     xls.costSummary['firstCellColumn'] + 1, formulaTotalDiskCost, selectBodyStyle)
customerVMDataExcelTab.write_formula(xls.costSummary['firstCellRow'] + xls.costSummary['rows']['ASR']['order'],
                                     xls.costSummary['firstCellColumn'] + 1, formulaTotalASRCost, selectBodyStyle)
customerVMDataExcelTab.write_formula(xls.costSummary['firstCellRow'] + xls.costSummary['rows']['TOTAL']['order'],
                                     xls.costSummary['firstCellColumn'] + 1, formulaTotalCost, selectHeaderStyle)
...
for column in xls.computeSummaryColumns['columns']:
    ...
    columnPositon = xls.computeSummaryColumns['firstColumnIndex'] + xls.computeSummaryColumns['columns'][column][
        'index']

    # SET WIDTH
    customerVMDataExcelTab.set_column(columnPositon, columnPositon, columnWidth)
    # SET HEADER
    customerVMDataExcelTab.write(xls.computeSummaryColumns['firstCellRow'], columnPositon, columnName,
                                 selectHeaderStyle)
    ...
    for rowIndex in range(1, xls.rowsForVMInput):
        if column == 'CHEAPEST SIZE':
            formula = formulaCheapestSizePattern.format(columnBestYearVMPrice, rowIndex + 1, columnYearPAYGVMPrice,
                                                        columnPAYGVMSize, columnYear1YResInsVMPrice,
                                                        column1YResInsVMSize, column3YResInsVMSize)
            ...
            formula = formulaCheapestModelPattern.format(columnBestYearVMPrice, rowIndex + 1, columnYearPAYGVMPrice,
                                                         columnYear1YResInsVMPrice)
...
customerVMDataExcelTab.conditional_format(cellRange,
                                          {'type': 'cell', 'criteria': 'equal to', 'value': '\"NO VM FOUND\"',
                                           'format': noVMFormat})
...
for size in sorted(computePriceMatrix, reverse=True):
    ...
    auxFormula = '=CONCATENATE(C{0},G{0},E{0},  IF(D{0}=\"YES\",\"OK\", \"KO\" ),  IF(I{0}=\"YES\",\"SDD\", \"HDD\" )  )'.format(
        currentLineBase + 1)
